	...
	switch(href_list["task"])
		if("random")
			switch(href_list["preference"])
				...
				if("underwear")
					underwear = get_valid_styles_from_cache(global.underwear_cache, species, gender)
				if("undershirt_options")
					...
					shirt_grad_style = random_undershirt_grad_style()
				...
				if("socks")
					...
					socks_grad = random_socks_grad()
					r_socks_grad = rand(0,255)
					g_socks_grad = rand(0,255)
					b_socks_grad = rand(0,255)
				if("eyes")
					r_eyes = rand(0,255)
					g_eyes = rand(0,255)
					b_eyes = rand(0,255)
				...
				if("undershirt_options")
					switch(tgui_alert(user, "What do you want to change?","Choose.", list("Type", "Color", "Picture", "Gradient")))
						...
						if("Picture")
							...
							if(valid_pics.len)
								var/new_pic = input(user, "Choose your undershirt picture:", "Character Preference", undershirt_pic) as null|anything in valid_pics
								if(new_pic)
